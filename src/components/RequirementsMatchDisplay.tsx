...
interface RequirementsMatchDisplayProps {
  requirements: RequirementMatch[];
  compact?: boolean;
}

export default function RequirementsMatchDisplay({ requirements, compact = false }: RequirementsMatchDisplayProps) {
                  ...
                  <div className="w-12 bg-gray-200 rounded-full h-1.5">
                    <div 
                      className={`h-1.5 rounded-full ${
                        req.match_score >= 80 ? 'bg-green-500' : 
                        req.match_score >= 60 ? 'bg-amber-500' : 'bg-red-500'
                      }`}
                      style={{ width: `${req.match_score}%` }}
                    />
                  </div>
                  <span className="text-xs font-medium">{req.match_score}%</span>
                </div>
              </div>
